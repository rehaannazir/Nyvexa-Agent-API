...
def update_user(user: UserNameUpdate, session=Depends(get_session)):

    user = AuthService.update_username(user.old_username, user.new_username, session)
    ...
    return {"message": f"Username is updated successfully to {user.new_username}"}


@router.post("/update-username", status_code=status.HTTP_200_OK)
def update_user(user: UserEmailUpdate, session=Depends(get_session)):

    user = AuthService.update_email(user.old_email, user.new_email, session)
    ...
    return {"message": f"Email is updated successfully to {user.new_email}"}
